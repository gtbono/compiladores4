...
bool declaracao_procedimento() {
	if (SEQ_TOKENS[NUM_TOKEN_ATUAL++].valor == PROCEDIMENTO) {
		...
		if (identificador(SEQ_TOKENS[NUM_TOKEN_ATUAL].valor) == true) {
			...
			TABELA_SIMBOLOS[INDICE_SIMBOLO].variavel_escopo = INDICE_ESCOPO;
			TABELA_SIMBOLOS[INDICE_SIMBOLO].variavel_tipo = PROCEDIMENTO;
			strcpy(TABELA_SIMBOLOS[INDICE_SIMBOLO++].variavel_nome, SEQ_TOKENS[NUM_TOKEN_ATUAL++].caracter);
			INDICE_ESCOPO++;
					...
					if (SEQ_TOKENS[NUM_TOKEN_ATUAL++].valor == PARENTESEDIR) {
						...
						if (bloco() == true) {
							if (SEQ_TOKENS[NUM_TOKEN_ATUAL++].valor == FIMPROCEDIMENTO)
								//Acredito que esteja certo por isso aqui para declaracao de variaveis depois do procedimento serem globais
								//INDICE_ESCOPO--;
								return true;
							else {
...
bool expressao() {
	if (expressao_simples() == true) {
		if (relacao(SEQ_TOKENS[NUM_TOKEN_ATUAL++].valor) == true) {
			if (tipoValido != INTEIRO) printf("Erro semantico, Operação com tipo invalido!");
			if (expressao_simples() == true) {
				return true;
			}
			else {
...
bool expressao_simples() {
	...
	if (temp == MAIS || temp == MENOS) {
		if (tipoValido != INTEIRO)printf("Erro semantico, atribuição de variaveis de tipos diferentes!");
		NUM_TOKEN_ATUAL++;
		if (termo() == true) {
			verif_exp = true;
			expressao_simples();
